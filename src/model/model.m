...
% ---- fill defaults (R2024b-friendly; no arguments-block) ----
...
net.init = @(scale) setfield(net, 'params', init_params(cfg, scale, cfg.dtype)); %#ok<SFLD>
end
...
for l = 1:Lh
    ...
    if nargin >= 4 && strcmp(mode,'train') && cfg.dropout > 0
        pkeep = 1 - cfg.dropout;
        % genera random sullo stesso device/tipo dell'array sottostante
